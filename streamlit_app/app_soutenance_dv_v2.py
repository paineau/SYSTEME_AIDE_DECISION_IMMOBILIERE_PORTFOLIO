...
def show_asset(title, filenames, caption=None, width=True):
    st.markdown(f"### {title}")
    for name in filenames:
        path = ASSETS_DIR / name
        if path.exists():
            st.image(str(path), use_container_width=width)
            if caption:
                st.caption(caption)
            return True
    st.info("Graphique à placer dans le dossier streamlit_app/assets : " + " ou ".join(filenames))
    return False

def insight_card(title, text):
    st.markdown(
        f'<div class="card"><h4>{title}</h4><p>{text}</p></div>',
        unsafe_allow_html=True
    )

def transition_box(text):
...
temporal_features_df = pd.DataFrame({
    ...
    "Score": [0.48, 0.25, 0.11, 0.09, 0.085, 0.06, 0.045, 0.015]
})
...
top_features_df = pd.DataFrame({
    "Variable": [
        ...
        "prix_m2_recalcule",
        "prix_m2",
        "geo_2000m_36m_surface_20_prix_m2_med",
        "geo_2000m_24m_surface_20_prix_m2_med",
        "geo_1000m_36m_surface_20_prix_m2_med",
        "geo_2000m_36m_same_type_prix_m2_med",
        "geo_1000m_24m_surface_20_prix_m2_med",
        "geo_500m_36m_surface_20_prix_m2_med",
        "geo_1000m_36m_same_type_prix_m2_med",
        "geo_500m_36m_same_type_prix_m2_med",
        "commune_type_prix_m2_moy_passe",
        "geo_2000m_12m_surface_20_prix_m2_med",
        "iris_prix_m2",
        "commune_type_prix_m2_med_passe"
    ],
    "Score": [1.0, 0.998, 0.996, 0.99, 0.985, 0.982, 0.975, 0.972, 0.968, 0.966, 0.963, 0.960, 0.958, 0.952, 0.948]
})

st.sidebar.title("Support de soutenance")
st.sidebar.caption("Version V2")
st.sidebar.markdown("🏠 Introduction")
st.sidebar.markdown("🧠 Construction du modèle")
st.sidebar.markdown("🏡 Démonstration")

page = st.sidebar.radio(
    "Parcours",
    [
        "🏠 Introduction · Problématique",
        "🏠 Introduction · Base analytique",
        "🧠 Construction du modèle · Raisonnement métier",
        "🧠 Construction du modèle · Scénarios",
        "🧠 Construction du modèle · Pouvoir prédictif",
        "🏡 Démonstration · Saisie",
        "🏡 Démonstration · Estimation",
        "🏡 Démonstration · Comparables",
        "🏡 Démonstration · Métropoles"
    ],
    key="parcours_soutenance_v2"
)
...
if page == "🏠 Introduction · Problématique":
    ...
    <div class="hero-box">
        ...
        <div class="hero-subtitle">Relier l'expertise métier, les données et la décision.</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    qui soit solide sur le plan analytique, explicable dans ses mécanismes et pertinente au regard des pratiques professionnelles ?
    <br><br>
    L'objectif n'est pas seulement de prédire un prix. Il est de comprendre ce qui le produit, de le formaliser
    et de mesurer la contribution réelle des familles d'information.
    </div>
    """, unsafe_allow_html=True)

    c1, c2, c3 = st.columns(3)
    with c1:
        insight_card("Métier", "La valeur immobilière est au cœur de l'investissement, du financement, du risque et de la décision.")
    with c2:
        insight_card("Données", "DVF apporte une profondeur nationale, mais doit être reconstruite pour devenir exploitable.")
    with c3:
        insight_card("Décision", "L'enjeu est de produire une lecture explicable, robuste et utilisable dans un contexte professionnel.")

    show_asset(
        "Schéma d'ensemble",
        ["01_problematique.png", "schema_problematique.png", "illustration_compagnon_immo.png"],
        "Ce visuel peut reprendre le lien entre expertise métier, données, data science et valorisation."
    )

elif page == "🏠 Introduction · Base analytique":
    st.title("Construction de la base analytique")
    ...
    fig = px.funnel(pipeline_df, x="Volume", y="Étape", title="De la ligne administrative à la base analytique")
    ...
    with c1:
        transition_box(
            "DVF ne décrit pas directement un bien immobilier. Le premier travail consiste à reconstruire l'unité économique de transaction."
        )
    with c2:
        transition_box(
            "L'horizon 2021-2025 permet de capter un cycle récent, avec rebond post-Covid, ajustement des volumes et remontée des taux."
        )

    show_asset(
        "Horizon temporel",
        ["02_volume_mensuel_transactions.png", "volume_mensuel_transactions.png"],
        "Graphique utile pour justifier la profondeur temporelle retenue."
    )

elif page == "🧠 Construction du modèle · Raisonnement métier":
    st.title("Formalisation du raisonnement métier")

    st.markdown("""
    <div class="big-question">
    La valeur n'est pas contenue dans le bien seul. Elle résulte d'une interaction entre caractéristiques physiques,
    territoire, temporalité, contexte socio-économique, financement et mémoire des ventes comparables.
    </div>
    """, unsafe_allow_html=True)
    ...
    with c2:
        show_asset(
            ...
            ["03_variations_prix_median.png", "variations_prix_median.png"]
        )

    transition_box(
        "Ces deux graphiques justifient la construction de variables temporelles : volume, momentum, variation, volatilité et tension de marché."
    )

    c1, c2 = st.columns([1.1, 1])
    with c1:
        fig = px.bar(
            temporal_features_df.sort_values("Score"),
            x="Score",
            y="Variable",
            orientation="h",
            title="Information mutuelle des variables temporelles"
        )
        fig.update_layout(height=420)
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        transition_box(
            "La sélection ne repose pas sur une intuition générale. Les variables sont comparées et hiérarchisées avant d'être intégrées aux scénarios."
        )

elif page == "🧠 Construction du modèle · Scénarios":
    st.title("Scénarios et résultats")

    st.markdown("""
    <div class="big-question">
    Les scénarios permettent de mesurer progressivement ce que chaque famille de données apporte réellement
    à l'explication du prix.
    </div>
    """, unsafe_allow_html=True)
    ...
    with c2:
        ...
        fig.update_layout(height=430, xaxis_range=[0, 0.82])
        st.plotly_chart(fig, use_container_width=True)

    fig = px.bar(
        top_features_df.sort_values("Score"),
        x="Score",
        y="Variable",
        orientation="h",
        title="Top features selon le score combiné"
    )
    fig.update_layout(height=620, xaxis_range=[0, 1.05])
    st.plotly_chart(fig, use_container_width=True)

    transition_box(
        "La hiérarchie des variables confirme le rôle central du territoire, des comparables et de l'historique local des prix."
    )

elif page == "🧠 Construction du modèle · Pouvoir prédictif":
    ...
    fig = px.line(
        ...
        color="Stratégie",
        markers=True,
        title="Comparaison des R² par scénario"
    )
    fig.update_layout(height=470, xaxis_tickangle=-35, yaxis_range=[-0.1, 0.85])
    st.plotly_chart(fig, use_container_width=True)

    c1, c2 = st.columns(2)
    with c1:
        show_asset(
            "Comparables vs modèle final",
            ["05_comparables_vs_modele_final.png", "comparables_vs_modele_final.png"]
        )
    with c2:
        show_asset(
            "Distribution des résidus",
            ["06_distribution_residus.png", "distribution_residus.png"]
        )

    transition_box(
        "La proximité entre split aléatoire et split temporel conforte la robustesse du dispositif. L'analyse des résidus complète la lecture en vérifiant la structure des erreurs."
    )

elif page == "🏡 Démonstration · Saisie":
...
elif page == "🏡 Démonstration · Estimation":
...
elif page == "🏡 Démonstration · Comparables":
...
elif page == "🏡 Démonstration · Métropoles":
    st.title("Marchés et métropoles")
    st.dataframe(metropoles_df, use_container_width=True, hide_index=True)

    fig1 = px.bar(
        metropoles_df.sort_values("Prix médian (€/m²)", ascending=False),
        x="Métropole",
        y="Prix médian (€/m²)",
        title="Prix médian au m² par métropole"
    )
    ...
    fig2 = px.scatter(
        ...
        y="Prix médian (€/m²)",
        size="Transactions",
        hover_name="Métropole",
        title="Volume et niveau de prix"
    )
